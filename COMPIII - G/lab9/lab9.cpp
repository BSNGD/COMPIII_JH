...
class PayKind { // base class - no member variables
	public:
		...
		virtual double payAdvice(){ return 0; }
...
class Salaried : public PayKind { // derived class
	public:
		...
		double payAdvice(){ _biweeklyPay = _annualSalary / 26; return _biweeklyPay;	}
...
class Hourly : public PayKind { // derived class
	public:
		...
		void output(ostream& out) const { out << *this; }
		double payAdvice(){ _biweeklyPay = _hourlyRate * 40; return _biweeklyPay;	}
...
class PerCourse : public PayKind { //derived class
	public:
		...
		void output(ostream& out) const { out << *this; }
		double payAdvice(){ _biweeklyPay = _feePerCourse / 7; return _biweeklyPay;	}
...
ostream& operator<< (ostream& out, const PerCourse& s) {
    out << "Fee per Course:\t" << s.getPerCourse() << endl;
...
class PartTime : public EmployeeKind { //derived class
	public:
		...
		~PartTime() { cout << "Salaried destructor called\n"; }
...
ostream& operator<< (ostream& out, const PartTime& s) {
    out << "Part time hours:" << s.getPartTimeHours() << endl;
...
class Adjunct : public Faculty, public PerCourse, public PartTime{
	public:
		...
		Adjunct(string firstName, string lastName, string address, unsigned _IDNumber,
				string SSN, string department, unsigned maxCourses, unsigned feePerCourse, 
				unsigned PartTimeHours);
		...
		// accessor functions
		unsigned getAmountCourses() const { return _maxCourses; }
...
Adjunct::Adjunct(string firstName, string lastName, string address, unsigned _IDNumber, string SSN, 
				 string department, unsigned maxCourses, unsigned feePerCourse, unsigned partTimeHours )
		: Faculty(firstName, lastName, address, _IDNumber, SSN, department), PerCourse(feePerCourse),
		PartTime(partTimeHours)
{
...
ostream& operator<< (ostream& out, const Adjunct& l) {
	l.Faculty::output(out); // display the Faculty variables

    out << "Max Courses:\t" << l.getAmountCourses() << endl;

	l.PerCourse::output(out);
	l.PartTime::output(out);
...
int main (void) {
	...
	Grader g1("John", "Doe", "89 Canal Street", 54329876, 3.5, "CS", "Tom Wilkes", "junior",
			  "COMP.2010", 25.00);
	cout << "Grader g1(John...):\n" << g1;
 	cout << "Bi-weekly Pay: " << g1.payAdvice() << endl;

	
	Grader g2(g1); // invoke copy constructor
	cout << "Grader g2(copy of g1 - John...):\n" << g2;
	cout << "Bi-weekly Pay: " << g2.payAdvice() << endl;
	...
	cout << "Bi-weekly Pay: " << n1.payAdvice() << endl;
	
	Adjunct a1("Javi", "Hernandez", "FooBar Street", 12345678, "321-54-9876", "Computer Science", 1, 7000, 20);
	cout << "Adjunct Faculty a1(Javi...):\n" << a1;
	cout << "Bi-weekly Pay: " << a1.payAdvice() << endl;
