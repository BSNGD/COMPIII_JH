...
//	createUMLPeopleDB(); // not needed for Lab 11 - output is already in file lab11_input.txt
	...
	it = find (upDB.begin(), upDB.end(), 87654321);
	if (it != upDB.end())
    	cout << "IDNumber 87654321 found in upDB for UMLPerson: " << *it;
    	
    it = std::find (upDB.begin(), upDB.end(), 89283002);
	if (it == upDB.end())    	
		cout << "IDNumber 89283002 not found in upDB" << endl;
	...
	auto resultid = minmax_element (upDB.begin(), upDB.end(), compare_IDs);
	cout << "min is " << *resultid.first;
	cout << "max is " << *resultid.second;
	cout << endl;
